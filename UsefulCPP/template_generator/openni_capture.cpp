...
void matToCSV (Mat& image, Rect detected_face )
{
	...
	else
	{
        filenumber++;
        std::ostringstream fileNameStream("");
        fileNameStream << "sparse/Capture" << filenumber - 1 << ".csv";
        name = fileNameStream.str();
		std::ostringstream fileNameStreamDense("");
        fileNameStreamDense << "dense/Capture" << filenumber - 1 << ".csv";
        nameDense = fileNameStreamDense.str();
	
        //name = "Final.csv";
	}
	//ofstream Face_Template ("Face_Template.csv"); 
	ofstream Face_Template (name.c_str()); 
	ofstream Face_Template_Dense (nameDense.c_str()); 
	int rows = image.rows;
	int cols = image.cols;
	...
	for (int i=0;i<rows;i++)
	{
		for (int j=0;j<cols;j++)
		{
			...
			if (depth != 0 && depth < (nose_depth + FACE_DEPTH))
			{
			      Face_Template_Dense << i + offset_y <<"," << j + offset_x <<"," <<depth + offset_z  << endl;
			}
		}
	}
...
{  
   ...
   cout<<"Detecting using Viola Jones"<<endl;
   }
   else {
   ...
   face_cascade.detectMultiScale( frame_gray, faces, 1.1, 4, 0|CV_HAAR_SCALE_IMAGE, Size(30, 30) );
     ...
     cout << " Golden rectangle is X : "<< golden_face.x << "    Y : " << golden_face.y << endl; 
...
//       }
   ...
   if (found == 1)
   {
	...
	else
	{
        ...
        //// If not converged == 1 then VIOLA JONES will be used 
        ////not_converged = 0;    
        //}catch(...){
