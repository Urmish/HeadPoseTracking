...
int line_from_icp_previous_input2=0;
int line_from_icp_input2;
unsigned int time_to_wait = 10;
unsigned int present_time = 0;

FILE* outfile;
int icp_has_input=0;
int getInputFromICP2()
{
    FILE* file_icp_input = fopen("/home/kush/Desktop/game/input_from_icp.txt", "r");
    fseek(file_icp_input, -2, SEEK_END);
    fscanf(file_icp_input, "%d", &line_from_icp_input2);
    //fscanf(f, "%d %d %d", &num1, &num2, &num3);


    fclose(file_icp_input);
    return 1;
    //if(line_from_icp_input2 != line_from_icp_previous_input2){
    //     line_from_icp_previous_input2 = line_from_icp_input2;
...
void checkinput()
{
    int value_from_icpx=0;
    int value_from_icpy=0;
    ...
    present_time++; 
    if(present_time >= time_to_wait){
        present_time = 0;
        icp_has_input = getInputFromICP2();
        fprintf(outfile,"Called getINputfrom ICP1 = %d\n",line_from_icp_input2);
      ...
      int value_from_icpy=0;
        if(icp_has_input==1){
         //int value_from_icp = atoi(line_from_icp_input2.c_str());
         //
         if(line_from_icp_input2==1){
             value_from_icpx = line_from_icp_input2*30;
             value_from_icpy =0;
         }else if(line_from_icp_input2==2){
             value_from_icpx = -1*line_from_icp_input2*30;
             value_from_icpy =0;
         }else if(line_from_icp_input2==3){
             value_from_icpx = 0;
             value_from_icpy = line_from_icp_input2*15;
         }else if(line_from_icp_input2==4){
             value_from_icpx = 0;
             value_from_icpy = -1*line_from_icp_input2*15;
           ...
           //cout << "Value from ICP = " << value_from_icp
           fprintf(outfile, "Mousemove called with value from icp : %d, %d\n", value_from_icpx, value_from_icpy);

    	   mousemove(value_from_icpx, value_from_icpy);
	       mousemoved = true;
        }
...
int main(int argc, char **argv)
{
    ...
    outfile = fopen("/home/kush/Desktop/game/output_from_game.txt", "w");
